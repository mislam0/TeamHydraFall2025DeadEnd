...
public class Player {
    ...
    private int previousRoomNumber;   // NEW: tracks the room you were in before the current one
    ...
    public Player() {
        ...
        this.previousRoomNumber = 1; // start with same as current
    ...
    // NEW: expose previous room (used conceptually for escape)
    ...
    public void setCurrentRoomNumber(int roomNumber, Map<Integer, Room> roomMap) {
        ...
        // Only update previous if we are actually changing rooms
        if (this.currentRoomNumber != roomNumber) {
            this.previousRoomNumber = this.currentRoomNumber;
        }
    ...
    // Enter a room (description + possible combat)
    public void enterRoom(Room room, Scanner scanner, Map<Integer, Room> roomMap) {
        ...
        // NOTE: we do NOT call setCurrentRoomNumber here anymore.
        // Movement (Player.move) and loading handle setting the current room.
        // This avoids overwriting previousRoomNumber.
            ...
            switch (choice) {
                ...
                case "ESCAPE":
                    ...
                    if (escaped) {
                        ...
                        int prevId = this.previousRoomNumber;
                        // valid previous room and not the same as current
                        if (roomMap != null &&
                            roomMap.containsKey(prevId) &&
                            prevId != this.currentRoomNumber) {

                            // Move back to previous room
                            setCurrentRoomNumber(prevId, roomMap);
                            Room prevRoom = roomMap.get(prevId);

                            System.out.println("You rush back to " + prevRoom.getName() + ".");

                            // Show description / possible combat / puzzle hints for that room
                            enterRoom(prevRoom, scanner, roomMap);
                            enterRoom(prevRoom, scanner);
                        } else {
                            System.out.println("But there is nowhere safe to run! You remain in place.");
                        }
