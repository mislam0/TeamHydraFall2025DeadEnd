...
public class Player {
    ...
    public Room move(String direction, Map<Integer, Room> roomMap) {
        // Resolve the player's current room from the canonical numeric id
        ...
        // Accept 0 as a valid room id; only reject null or non-existent ids
        if (nextRoomNumber == null || !roomMap.containsKey(nextRoomNumber)) {
            System.out.println("You can't go this way.");
            return null;
        }
    
        // Update both the numeric id and the Room reference so later commands see the new room
        setCurrentRoomNumber(nextRoomNumber, roomMap);
    
        // Return the new current room (field was set by the setter)
        return this.currentRoom;
    }

    // Keep this setter in the same class. It validates and keeps both fields in sync.
            ...
            switch (choice) {
                ...
                case "ESCAPE":
                    ...
                    if (escaped) {
                        ...
                        // Use roomMap.containsKey so 0-based IDs are allowed and we only accept real targets
                        for (Map.Entry<String, Integer> entry : exits.entrySet()) {
                            Integer targetId = entry.getValue();
                            if (targetId != null && roomMap.containsKey(targetId)) {
                                validDirs.add(entry.getKey());
                            }
                        }

                        if (!validDirs.isEmpty()) {
                            int roll = dice.dX(validDirs.size()) - 1; // dice.dX returns 1..n
                            String chosenDir = validDirs.get(roll);
